...
public class Classifier {
    ...
    public void get() {
        ...
        String probability = " " + (int) (probabilities[maxAt] * 100) + "%";

        if (translator.isReadyToTranslate()) {
            translator.translateWord(lastProbability.get(maxAt).getLabel(), responseHandler, probability);
        } else {
            Message msg = new Message();
            msg.obj = lastProbability.get(maxAt).getLabel() + probability;
